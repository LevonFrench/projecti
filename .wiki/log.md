# Wiki Activity Log

## [2026-06-23] init | Wiki initialized with standard Topics directory structure
## [2026-06-23] research | Completed deep research on Steve Jackson's classic Illuminati card game cards, stats, alignments, and actions, outlining adaptations for our new card game project.
## [2026-06-23] compile | Compiled comprehensive Illuminati Gameplay Mechanics (mechanics.md) detailing card types, turn phase state machines, attack math, and trade/privilege protocols.
## [2026-06-23] compile | Expanded knowledge base on Steve Jackson's Illuminati card game with new articles for INWO CCG (inwo.md), classic game expansions (expansions.md), and strategy guides (strategies.md).
## [2026-06-23] research | Researched game history, Steve Jackson's biography, and the 1990 Secret Service raid; compiled history_and_creator.md and editions_and_variants.md; downloaded 21 official card art preview assets to assets/ directory.
## [2026-06-23] website | Built and verified reference archive website (Atelier Zero aesthetic) showcasing the Illuminati KB and card art preview bento grid.
## [2026-06-23] ingest | Illuminati Card Game Part I - Complete List (raw/papers/2026-06-23-illuminati-card-game-part-i-complete-list.md)
## [2026-06-23] ingest | Illuminati Card Game Part II - Complete List (raw/papers/2026-06-23-illuminati-card-game-part-ii-complete-list.md)
## [2026-06-23] audit | scope=website, outputs=1, drifted=0, escalations=1
## [2026-06-23] extract | Programmatically extracted 496 high-quality card images from the ingested Illuminati scan PDFs into the topics assets directory.
## [2026-06-23] compile | Updated compile_kb.js to index card images from the wiki and automatically copy/synchronize them to the website assets folder, and resolved app.js file extension formatting.
## [2026-06-23] research | Researched game duration, collected session logs, and compiled core specs: turn_sequence_details.md, stats_tracking.md, rule_logic_spec.md, and backend_architecture_plan.md.
## [2026-06-23] ingest | Ingested playtime research and sample session transcript recap under raw/papers/.
## [2026-06-23] build | Customized card design editor (Cider) with the compiled 192 card Illuminati database, attributes, templates, and production build.
## [2026-06-23] deploy | Bundled reference website to `/website/` path in Cider build assets, recompiled clean website data, and deployed production release to Vercel.

